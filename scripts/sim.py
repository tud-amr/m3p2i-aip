...
class SIM():
    def __init__(self, params) -> None:
        ...
        self.gym, self.sim, self.viewer, self.envs, _ = sim_init.make(params, "sim")
    ...
    def reset(self):
        ...
        if self.environment_type == 'cube':
            ...
            cubeB_index = 2
        obs_index = 6
        x_pos = torch.tensor([0.03, 0, 0], **params.tensor_args).repeat(self.num_envs)
        y_pos = torch.tensor([0, 0.03, 0], **params.tensor_args).repeat(self.num_envs)
        z_pos = torch.tensor([0, 0, 0.03], **params.tensor_args).repeat(self.num_envs)
